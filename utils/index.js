...
export const getImageSrc = ($, style) => {
  const divs = $('div.vendor-image')

  const srcs = divs.children('img').map((i, el) => $(el).attr('src'))

  const src = srcs.filter((i, el) => el.includes(style))

  return src[0]
}

export const isValidEmoji = emoji => {
  return emoji.match(emojiREGEX) !== null
}
